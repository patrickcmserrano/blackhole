<script lang="ts">
  ...
  // Check URL query parameters for splash mode and initial scene
  const urlParams = new URLSearchParams(typeof window !== "undefined" ? window.location.search : "");
  const initialScene = urlParams.get("scene") as SceneType;
  const isSplash = urlParams.get("splash") === "true";

  let currentScene = $state<SceneType>(
    (["blackhole", "neutronstar", "saturn", "nebula"].includes(initialScene))
      ? initialScene
      : "blackhole"
  );
